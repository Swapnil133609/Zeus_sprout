...
static void synaptics_rmi4_f1a_report(struct synaptics_rmi4_data *rmi4_data,
		struct synaptics_rmi4_fn *fhandler)
{
printk("[s3508]_synaptics_rmi4_f1a_repor");
	int retval;
...
printk("[s3508_14]_button ==%d\n",button);
printk("[s3508_14]_button f1a->valid_button_count ==%d\n", f1a->valid_button_count);
	for (button = 0; button < f1a->valid_button_count; button++) {
		index = button / 8;
		shift = button % 8;
		status = ((data[index] >> shift) & MASK_1BIT);
printk("[s3508_14]_for_button ==%d\n",button);
printk("[s3508_14]_status ==%d\n",status);
...
printk("[s3508_14]_button_for_end ==%d\n",button);
	if (touch_count)
		input_sync(rmi4_data->input_dev);
...
static int touch_event_handler(void *data)
{
	...
	printk("synaptics_touch_event_handler");
...
static void synaptics_rmi4_power_suspend(struct power_suspend *h)
{
	...
	if (!list_empty(&exp_data.list)) {
		list_for_each_entry(exp_fhandler, &exp_data.list, link)
			if (exp_fhandler->exp_fn->power_suspend != NULL)
				exp_fhandler->exp_fn->power_suspend(rmi4_data);
	}
	mutex_unlock(&exp_data.mutex);
...
static void synaptics_rmi4_power_resume(struct power_suspend *h)
{
	...
	if (!list_empty(&exp_data.list)) {
		list_for_each_entry(exp_fhandler, &exp_data.list, link)
			if (exp_fhandler->exp_fn->power_resume != NULL)
				exp_fhandler->exp_fn->power_resume(rmi4_data);
	}
	mutex_unlock(&exp_data.mutex);
