...
static void get_module_name(int sensor_id, char *module_name)
{
	if (tp_modules[sensor_id])
		strncpy(module_name, tp_modules[sensor_id], sizeof(tp_modules[sensor_id]));
	else
		strncpy(module_name, "unknow", sizeof("unknow"));
}
...
s32 gtp_i2c_read_dbl_check(struct i2c_client * client, u16 addr, u8 * rxbuf, int len)
{
	...
	while (retry++ < 3) {
		...
		if ((len + 2) > 16) {
			GTP_ERROR("bad len for transfer:(%d)!", len);
			return FAIL;
		}
...
s32 gtp_read_version(struct i2c_client * client, u16 * version)
{
	...
	if (buf[5] == 0x00) {
		...
		snprintf(ic_g, "%c%c%c", buf[2], buf[3], buf[4]);
	} else {
		if (buf[5] == 'S' || buf[5] == 's') {
			chip_gt9xxs = 1;
		}
		...
		snprintf(ic_g, "%c%c%c%c", buf[2], buf[3], buf[4], buf[5]);
	}
	return ret;
}
...
static s32 gtp_init_panel(struct i2c_client *client)
{
	...
	u8 check_sum = 0;
	u8 opr_buf[16];
	/* u8 sensor_id = 0;//modify by juwei */
	u8 retry = 0;
...
/* #ifdef VELOCITY_CUSTOM */
	...
	if (!int_type) {
		...
		mt65xx_eint_registration(CUST_EINT_TOUCH_PANEL_NUM,
					 CUST_EINT_TOUCH_PANEL_DEBOUNCE_EN, CUST_EINT_POLARITY_LOW,
					 tpd_eint_interrupt_handler, 1);
	}
#endif
...
/* liupeng 2015-3-20 [][FIX BUG:TP info shows error in 9375 mode]start */
	gtp_read_version(client, &ic_fw_version_g);
	sensor_id_g = sensor_id;
	get_module_name(sensor_id_g, module_name_g);

	if (strncmp(ic_g, "915M", 4) == 0) {
#ifdef RGK_SAMPLE_GT915M_VERSION
		ver = RGK_SAMPLE_GT915M_VERSION;
#else
		ver = 0xffff;
#endif
	} else if (strncmp(ic_g, "9147", 4) == 0) {
#ifdef RGK_SAMPLE_GT9147_VERSION
		ver = RGK_SAMPLE_GT9147_VERSION;
#else
		ver = 0xffff;
#endif
	} else {
	...
	len =
	    sprintf(tp_info,
		    "TP IC:GT%s,TP MODULE:%s,TP I2C ADR:0x%x,SW FirmWare:0x%x,Sample FirmWare:0x%x",
		    ic_g, module_name_g, client->addr, ic_fw_version_g, ver);
...
static int tpd_local_init(void)
{
	...
	if (touch_ssb_data.use_tpd_button == 1) {
		tpd_button_setting(TPD_KEY_COUNT, touch_ssb_data.tpd_key_local,
				   touch_ssb_data.tpd_key_dim_local);
	}
#if (defined(TPD_WARP_START) && defined(TPD_WARP_END))
	TPD_DO_WARP = 1;
...
static int __init tpd_driver_init(void)
{
	...
	GTP_INFO
	    ("gt915 touch_ssb_data:: name:(%s), endflag:0x%x, i2c_number:0x%x, i2c_addr:0x%x,power_id:%d, use_tpd_button:%d\n",
	     touch_ssb_data.identifier, touch_ssb_data.endflag, touch_ssb_data.i2c_number,
	     touch_ssb_data.i2c_addr, touch_ssb_data.power_id, touch_ssb_data.use_tpd_button);
