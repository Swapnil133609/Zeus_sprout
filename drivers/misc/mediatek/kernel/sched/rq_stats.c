...
unsigned int sched_get_nr_heavy_task_by_threshold(unsigned int threshold)
{
	...
	unsigned int hmp_threshold = threshold;
	...
	for_each_online_cpu(cpu) {
		...
		{
			hmp_threshold = threshold * HMP_RATIO;
		}
		else
		{
			hmp_threshold = threshold;
		}
#endif
		...
		list_for_each_entry(p, &cpu_rq(cpu)->cfs_tasks, se.group_node) {
			...
			{
/*
				printk_sched("htask=%s(%d) load=%lu, e=%d, r=%d, l=%d\n", p->comm, p->pid, p->se.avg.load_avg_ratio,
					p->se.avg.usage_avg_sum, p->se.avg.runnable_avg_sum, p->se.avg.runnable_avg_period);
*/
	...
	if (count)
	{
		if (htask_temperature <= MAX_HTASK_TEMPERATURE)
			htask_temperature++;
	return count;
	}
	else // count==0
	{
		if (htask_temperature)
		{
			htask_temperature--;
			return 1;
		}
