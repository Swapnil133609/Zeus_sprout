...
static int lsm303m_remove(void);
extern struct mag_hw* lsm303d_get_cust_mag_hw(void);

static struct mag_init_info lsm303m_init_info = {
        .name = "lsm303d_mag",
        .init = lsm303m_local_init,
        .uninit = lsm303m_remove,
...
/*----------------------------------------------------------------------------*/
#define MSE_TAG                    "[LSM303D_MAG] "
#define MSE_FUN(f)                 pr_notice(MSE_TAG"%s\n", __func__)
#define MSE_ERR(fmt, args...)      pr_err(MSE_TAG"%s %d : "fmt, __func__, __LINE__, ##args)
#define MSE_LOG(fmt, args...)      pr_notice(MSE_TAG fmt, ##args)
#define MSE_WARN(fmt, args...)     pr_warn(MSE_TAG"%s: "fmt, __func__, ##args)
...
/*----------------------------------------------------------------------------*/
static void lsm303m_power_suspend(struct power_suspend *h)
{
    struct lsm303m_i2c_data *obj = container_of(h, struct lsm303m_i2c_data, power_drv);
...
/*----------------------------------------------------------------------------*/
static void lsm303m_power_resume(struct power_suspend *h)
{
    struct lsm303m_i2c_data *obj = container_of(h, struct lsm303m_i2c_data, power_drv);
...
static int lsm303m_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
        ...
        data->hw = lsm303d_get_cust_mag_hw();
        ...
        if((err = lsm303m_create_attr(&(lsm303m_init_info.platform_diver_addr->driver))))
        {
        ...
        sobj_m.self = data;
        sobj_m.polling = 1;
        sobj_m.mag_operate = lsm303m_operate;
        ...
        sobj_o.self = data;
        sobj_o.polling = 1;
        sobj_o.mag_operate = lsm303m_orientation_operate;
        ...
        data->power_drv.suspend  = lsm303m_power_suspend,
        data->power_drv.resume     = lsm303m_power_resume,
...
static int lsm303m_i2c_remove(struct i2c_client *client)
{
    ...
    if((err = lsm303m_delete_attr(&(lsm303m_init_info.platform_diver_addr->driver))))
    {
