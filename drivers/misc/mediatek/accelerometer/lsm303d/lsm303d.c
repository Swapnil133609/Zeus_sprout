...
static int lsm303d_acc_local_init(void);
static int lsm303d_remove(void);

static struct acc_init_info lsm303d_acc_init_info = {
        .name = "lsm303d",
        .init = lsm303d_acc_local_init,
        .uninit = lsm303d_remove,
...
static int lsm303d_init_flag =-1; // 0<==>OK -1 <==> fail
...
#endif
static int lsm303d_enable_nodata(int en)
{
    int res =0;
    int retry = 0;
    bool power=false;
    ...
    for(retry = 0; retry < 3; retry++){
        res = lsm303d_SetPowerMode(obj_i2c_data->client, power);
        if(res == 0)
        {
            GSE_LOG("lsm303d_SetPowerMode done\n");
            break;
        }
        GSE_LOG("lsm303d_SetPowerMode fail\n");
    }


    if(res != LSM303D_SUCCESS)
    {
        printk("lsm303d_SetPowerMode fail!\n");
        return -1;
    }
    printk("lsm303d_enable_nodata OK!\n");
    return 0;
}

static int lsm303d_open_report_data(int open)
{
    return 0;
}

static int lsm303d_set_delay(u64 ns)
{
    int value =0;
    int sample_delay=0;
    int err;
    ...
    if(value <= 5)
    {
        sample_delay = LSM303D_ACC_ODR200;
    }
    else if(value <= 10)
    {
        sample_delay = LSM303D_ACC_ODR100;
    ...
    err = lsm303d_SetBWRate(obj_i2c_data->client, sample_delay);
    if(err )
    {
        GSE_ERR("lsm303d_set_delay Set delay parameter error!\n");
        return -1;
    }
    GSE_LOG("lsm303d_set_delay (%d)\n",value);
    return 0;
}

static int lsm303d_get_data(int* x ,int* y,int* z, int* status)
{
    char buff[LSM303D_BUFSIZE];

    lsm303d_ReadSensorData(obj_i2c_data->client, buff, LSM303D_BUFSIZE);
...
static int lsm303d_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    obj->hw = lsm303d_get_cust_acc_hw();
    ...
    if((err = lsm303d_create_attr(&(lsm303d_acc_init_info.platform_diver_addr->driver))))
    {
        GSE_ERR("create attribute err = %d\n", err);
        goto exit_create_attr_failed;
    }

    ctl.open_report_data= lsm303d_open_report_data;
    ctl.enable_nodata = lsm303d_enable_nodata;
    ctl.set_delay  = lsm303d_set_delay;
    ...
    data.get_data = lsm303d_get_data;
...
static int lsm303d_i2c_remove(struct i2c_client *client)
{
    ...
    if((err = lsm303d_delete_attr(&(lsm303d_acc_init_info.platform_diver_addr->driver))))
    {
